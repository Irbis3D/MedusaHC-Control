...
function renderSettings(payload) {
  ...
  if (app.settingsPage === "tuning") {
    $("#tuning-groups").innerHTML = `<section class="tuning-guide"><div><strong>Apply</strong><span>Changes the running value immediately and resets after restart.</span></div><div><strong>Save to config</strong><span>Permanently replaces the stored value after confirmation.</span></div></section>${toolPanel}<div class="global-tuning-groups">${general}</div>`;
  } else {
